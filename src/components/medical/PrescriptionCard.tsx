...
import type { Prescription, PrescriptionStatus } from '@/types/medical.types';

interface PrescriptionCardProps {
  prescription: Prescription;
}

const STATUS_MAP: Record<PrescriptionStatus, { label: string; color: string; bg: string }> = {
  PENDING: { label: 'Chờ xử lý', color: 'text-slate-600', bg: 'bg-slate-100' },
  ACTIVE: { label: 'Đang áp dụng', color: 'text-blue-600', bg: 'bg-blue-100' },
  COMPLETED: { label: 'Hoàn thành', color: 'text-green-600', bg: 'bg-green-100' },
  CANCELLED: { label: 'Đã hủy', color: 'text-red-600', bg: 'bg-red-100' },
  EXPIRED: { label: 'Hết hạn', color: 'text-orange-600', bg: 'bg-orange-100' },
};
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h3 className="font-bold text-slate-900">{prescription.prescriptionNumber}</h3>
              <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${status.bg} ${status.color}`}>
                {status.label}
              </span>
            </div>
            <p className="text-sm text-slate-500 mb-2">
              Chẩn đoán: <span className="text-slate-700 font-medium">{prescription.diagnosis || 'N/A'}</span>
            </p>
            <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-slate-500">
              <div className="flex items-center gap-1">
                <span className="material-symbols-outlined text-sm">person</span>
                Bác sĩ: {prescription.doctor?.fullName || 'N/A'}
              </div>
              <div className="flex items-center gap-1">
                <span className="material-symbols-outlined text-sm">calendar_today</span>
                Ngày kê đơn: {format(new Date(prescription.createdAt), 'dd/MM/yyyy', { locale: vi })}
              </div>
