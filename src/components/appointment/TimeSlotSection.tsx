...
// ✅ Fix: chỉ có VIDEO và IN_CLINIC theo enum BE
function slotMatchesAppointmentType(
  slot: TimeSlot,
  appointmentType: "all" | "online" | "offline",
) {
  ...
  if (
    ...
    slot.allowedAppointmentTypes.length === 0
  )
    return true;

  const normalized = slot.allowedAppointmentTypes.map((item) =>
    item.toUpperCase(),
  );
  ...
  // offline = IN_CLINIC
  return normalized.includes("IN_CLINIC");
}
...
function TimeSlotSection({
  ...
  const morningSlots = useMemo(
    () => slots.filter((slot) => getVietnamHour(slot.startTime) < 12),
    [slots],
  );
  ...
  const displayedSlots =
    activeSession === "morning" ? morningSlots : afternoonSlots;
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
      ...
      <div className="bg-slate-100 dark:bg-slate-800 p-1 rounded-xl flex mb-6">
        ...
        >
          Buổi chiều ({afternoonSlots.length})
        </button>
      </div>
      ...
      {selectedDate && !slotsQuery.isLoading && displayedSlots.length === 0 && (
        <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 py-10 text-center text-slate-500">
          {activeSession === "morning"
            ? "Không có khung giờ buổi sáng"
            : "Không có khung giờ buổi chiều"}
        </div>
      )}
          ...
          {displayedSlots.map((slot) => {
            ...
            return (
              ...
              >
                ...
                >
                  {formatVietnamTime(slot.startTime)} -{" "}
