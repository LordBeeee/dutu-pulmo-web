...
function TimeSlotSection({
    ...
    useEffect(() => {
        ...
        fetch(
            `https://dutu-pulmo-be.onrender.com/public/doctors/${doctorId}/time-slots/available?date=${selectedDate}`
        )
            .then((res) => {
                if (!res.ok) throw new Error("Không lấy được khung giờ khám");
                return res.json();
            })
            .then((data) => {
                setSlots(data.data || []);
            ...
            .catch((error) => {
                console.error("Lỗi lấy time slots:", error);
                setSlots([]);
            })
            .finally(() => {
                setLoading(false);
    ...
    const morningSlots = useMemo(() => {
        return slots.filter((slot) => getVietnamHour(slot.startTime) < 12);
    }, [slots]);
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm">
            ...
            {selectedDate && !loading && displayedSlots.length === 0 && (
                <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 py-10 text-center text-slate-500">
                    {activeSession === "morning"
